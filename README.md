...
1. Run **Calva: Start a project REPL and Connect (aka Jack-in)** from the command pallet.
1. Select **shadow-cljs** as project type.
1. Select **:app** as build to start.
1. Select **:app** as build to connect to.
...
* [`src/fee_calculator/`](src/fee_calculator/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  * [`core.cljs`](src/fee_calculator/core.cljs): contains the SPA entry point, `init`
* [`test/fee_calculator/`](test/fee_calculator/): test files (ClojureScript,
...
1. Install [Node.js](https://nodejs.org/) (JavaScript runtime environment) which should include
   [NPM](https://docs.npmjs.com/cli/npm) or if your Node.js installation does not include NPM also install it.
1. Install [Chrome](https://www.google.com/chrome/) or
...
1. Clone this repo and open a terminal in the `fee-calculator` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/fee_calculator/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns fee-calculator.example
  (:require [fee-calculator.config :as config])
